                              ...
                              uint32_t                   *pProfile)
{
   ...
   status_v = MATHLIB_compare_mem(pDst, (T *) pCurrPrm->staticOut, length, 10e-5, testMetric);
   ...
   /* Cold run */
   pDst[0] = MATHLIB_atan2_scalar_ci<T>(pSrc0[0], pSrc1[0]);
   pDst[0] = MATHLIB_atan2_scalar_ci<T>(pSrc0[0], pSrc1[0]);

   /* Warm run*/
   for (size_t ii = 0; ii < length; ii++) {
      MATHLIB_profile_start(MATHLIB_PROFILE_KERNEL_CN);
      pDst[ii] = MATHLIB_atan2_scalar_ci<T>(pSrc0[ii], pSrc1[ii]);
      MATHLIB_profile_stop();
   }

   status_inline = MATHLIB_compare_mem(pDst, (T *) pCurrPrm->staticOut, length, 10e-5, testMetric);
...
int MATHLIB_atan2_testBench(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   for (tpi = 0; tpi < (size_t) test_cases; tpi++) {
      ...
      if (currPrm.dType == MATHLIB_FLOAT32) {
         ...
         status =
             MATHLIB_atan2_runTest(&currPrm, length, (double *) pSrc0, (double *) pSrc1, (double *) pDst, &pProfile[0]);
      }
...
int coverage_test_main(uint32_t *pProfile)
{
   ...
   while (testNum <= 1016) {
      switch (testNum) {
      case 1000:
         status          = MATHLIB_atan2(16, (float *) NULL, pSrc1_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1001:
         status          = MATHLIB_atan2(16, pSrc0_sp, (float *) NULL, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1002:
         status          = MATHLIB_atan2(16, pSrc0_sp, pSrc1_sp, (float *) NULL);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1003:
         status          = MATHLIB_atan2(0, pSrc0_sp, pSrc1_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1004:
         pSrc0_sp[0]     = FLT_MAX;
         status          = MATHLIB_atan2_sp(1, pSrc0_sp, pSrc1_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1005:
         status          = MATHLIB_atan2_dp(0, pSrc0_dp, pSrc1_dp, pDst_dp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1006:
         pSrc0_dp[0]     = 0.0f;
         pSrc1_dp[0]     = 0.0f;
         status          = MATHLIB_atan2_dp(1, pSrc0_dp, pSrc1_dp, pDst_dp);
         currentTestPass = (pDst_dp[0] == 0.0f);
         break;
      case 1007:
         pSrc0_dp[0]     = 1.0f / 0.0f;
         pSrc1_dp[0]     = 1.0f;
         status          = MATHLIB_atan2_dp(1, pSrc0_dp, pSrc1_dp, pDst_dp);
         currentTestPass = (pDst_dp[0] == HalfPI);
         break;
      case 1008:
         pSrc0_dp[0]     = -1.0f / 0.0f;
         pSrc1_dp[0]     = 1.0f;
      ...
      case 1012:
         ...
         pSrc1_sp[0]     = 0.0f;
         status          = MATHLIB_atan2_sp(1, pSrc0_sp, pSrc1_sp, pDst_sp);
         currentTestPass = (pDst_sp[0] == 0.0f);
         break;
      case 1013:
         pSrc0_sp[0]     = 0.0f;
         pSrc1_sp[0]     = -1.0f;
         status          = MATHLIB_atan2_sp(1, pSrc0_sp, pSrc1_sp, pDst_sp);
         currentTestPass = (pDst_sp[0] == pi);
         break;
      case 1014:
         pSrc0_sp[0]     = -1.0f;
         pSrc1_sp[0]     = 0.0f;
         status          = MATHLIB_atan2_sp(1, pSrc0_sp, pSrc1_sp, pDst_sp);
         currentTestPass = (pDst_sp[0] == (-pih));
         break;
      case 1015:
         pSrc0_sp[0]     = 1.0f;
         pSrc1_sp[0]     = 0.0f;
...
int main(void)
{
   ...
   test_fail = MATHLIB_atan2_testBench(&profile[0], 0);
