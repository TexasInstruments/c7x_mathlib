...
#include "MATHLIB_recip/MATHLIB_recip_scalar.h"
#include "MATHLIB_recip_testParams.h"
...
/******************************************************************************/
/*                                                                            */
/* MATHLIB_recip_runTest                                                      */
/*                                                                            */
/******************************************************************************/

// this method tests the kernel for a given test case
template <typename T>
int32_t MATHLIB_recip_runTest(MATHLIB_recip_testParams_t *pCurrPrm, size_t length, T *pSrc, T *pDst, uint32_t *pProfile)
{
   ...
   MATHLIB_recip(length, pSrc, pDst);
   ...
   if (pCurrPrm->dType == MATHLIB_FLOAT64) {
      tolerance = 1e-15;
   }
   status = MATHLIB_compare_mem(pDst, (T *) pCurrPrm->staticOut, length, tolerance, testMetric);
...
/******************************************************************************/
/*                                                                            */
/* MATHLIB_recip_testBench                                                    */
/*                                                                            */
/******************************************************************************/

// this method performs regression testing for the MATHLIB_recip kernel
int MATHLIB_recip_testBench(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   // variables to keep track of test cases
   size_t                      tpi = 0; // test case iteration variable
   MATHLIB_recip_testParams_t *prm;     // pointer to test params struct
   MATHLIB_recip_testParams_t  currPrm; // current test case
   ...
   void  *pSrc   = NULL; // pointer to input vector; assigned later
   void  *pDst   = NULL; // pointer to output vector; assigned later

   // get total number of test case
   MATHLIB_recip_getTestParams(&prm, &test_cases);

   // name of profile
   MATHLIB_profile_init((char *) "MATHLIB_recip");
   ...
   for (tpi = 0; tpi < (size_t) test_cases; tpi++) {
      ...
      if (currPrm.dType == MATHLIB_FLOAT32) {
         ...
         status = MATHLIB_recip_runTest(&currPrm, length, (float *) pSrc, (float *) pDst, &pProfile[0]);
      }
      else if (currPrm.dType == MATHLIB_FLOAT64) {

         status = MATHLIB_recip_runTest(&currPrm, length, (double *) pSrc, (double *) pDst, &pProfile[0]);
      }
...
int coverage_test_main(uint32_t *pProfile)
{
   ...
   while (testNum <= 1003) {
      switch (testNum) {
      case 1000:
         status          = MATHLIB_recip(16, (float *) NULL, pDst);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1001:
         status          = MATHLIB_recip(16, pSrc, (float *) NULL);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1002:
         status          = MATHLIB_recip(0, pSrc, pDst);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1003:
         status          = MATHLIB_recip_sp(8, pSrc, pDst);
   ...
   /* For every call of MATHLIB_recip() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling MATHLIB_recip() to push the execution history
      at the end of coverage test cases. */
   MATHLIB_recip(0, pSrc, pDst);
   printf("!!! Pushing final execution history for MATHLIB_recip()\n");
#endif
...
int main(void)
{
   ...
   test_fail = MATHLIB_recip_testBench(&profile[0], 0);
