...
int32_t MATHLIB_sqrt_runTest(MATHLIB_sqrt_testParams_t *pCurrPrm, size_t length, T *pSrc, T *pDst, uint32_t *pProfile)
{
   ...
   for (size_t ii = 0; ii < length; ii++) {
      ...
      pDst[ii] = MATHLIB_sqrt_scalar_ci<T>(pSrc[ii]);
      MATHLIB_profile_stop();
   }
...
int MATHLIB_sqrt_testBench(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   for (tpi = 0; tpi < (size_t) test_cases; tpi++) {
      ...
      if (currPrm.dType == MATHLIB_FLOAT32) {
         ...
         status = MATHLIB_sqrt_runTest(&currPrm, length, (double *) pSrc, (double *) pDst, &pProfile[0]);
      }
...
int coverage_test_main(uint32_t *pProfile)
{
   ...
   while (testNum <= 1006) {
      switch (testNum) {
      case 1000:
         status          = MATHLIB_sqrt(16, (float *) NULL, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1001:
         status          = MATHLIB_sqrt(16, pSrc_sp, (float *) NULL);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1002:
         status          = MATHLIB_sqrt(0, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1003:
         status          = MATHLIB_sqrt_sp(16, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1004:
         status          = MATHLIB_sqrt_dp(16, pSrc_dp, pDst_dp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1005:
         pSrc_sp[0]      = 1.0f / 0.0f;
         status          = MATHLIB_sqrt(1, pSrc_sp, pDst_sp);
         currentTestPass = (pDst_sp[0] == FLT_MAX);
         break;
      case 1006:
         pSrc_dp[0]      = 1.0f / 0.0f;
         status          = MATHLIB_sqrt(1, pSrc_dp, pDst_dp);
         currentTestPass = (pDst_dp[0] == DBL_MAX);
         break;
      default:
         break;
      }
...
int main(void)
{
   ...
   test_fail = MATHLIB_sqrt_testBench(&profile[0], 0);
