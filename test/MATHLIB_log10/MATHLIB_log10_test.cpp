                              ...
                              uint32_t *restrict pProfile)
{
   ...
   status_v = MATHLIB_compare_mem(pDst, (T *) pCurrPrm->staticOut, length, 10e-2, testMetric);
   ...
   for (size_t ii = 0; ii < length; ii++) {
      ...
      pDst[ii] = MATHLIB_log10_scalar_ci<T>(pSrc[ii]);
      MATHLIB_profile_stop();
   }

   status_inline = MATHLIB_compare_mem(pDst, (T *) pCurrPrm->staticOut, length, 10e-2, testMetric);
...
int MATHLIB_log10_testBench(uint32_t *restrict pProfile, uint8_t LevelOfFeedback)
{
   ...
   for (tpi = 0; tpi < (size_t) test_cases; tpi++) {
      ...
      if (currPrm.dType == MATHLIB_FLOAT32) {
         ...
         status = MATHLIB_log10_runTest(&currPrm, length, (double *) pSrc, (double *) pDst, &pProfile[0]);
      }
...
int coverage_test_main(uint32_t *restrict pProfile)
{
   ...
   while (testNum <= 1008) {
      switch (testNum) {
      case 1000:
         status          = MATHLIB_log10(16, (float *) NULL, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1001:
         status          = MATHLIB_log10(16, pSrc_sp, (float *) NULL);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1002:
         status          = MATHLIB_log10(0, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1003:
         pSrc_sp[0]      = FLT_MAX;
         status          = MATHLIB_log10_sp(1, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1004:
         pSrc_sp[0]      = 0.0;
         status          = MATHLIB_log10(1, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1005:
         pSrc_dp[0]      = 1.0f / 0.0f;
         status          = MATHLIB_log10_dp(1, pSrc_dp, pDst_dp);
         currentTestPass = (pDst_dp[0] == 308.254715974092);
         break;
      case 1006:
         pSrc_dp[0]      = 0.0f;
         status          = MATHLIB_log10_dp(1, pSrc_dp, pDst_dp);
         currentTestPass = (pDst_dp[0] == (-1.7976931348623157e+308));
         break;
      case 1007:
         pSrc_sp[0]      = 1.0f / 0.0f;
         status          = MATHLIB_log10_sp(1, pSrc_sp, pDst_sp);
         currentTestPass = (pDst_sp[0] == 308.2547f);
         break;
      case 1008:
         pSrc_dp[0]      = DBL_MAX;
         status          = MATHLIB_log10_dp(1, pSrc_dp, pDst_dp);
...
int main(void)
{
   ...
   test_fail = MATHLIB_log10_testBench(&profile[0], 0);
