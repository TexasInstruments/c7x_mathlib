...
int32_t MATHLIB_sin_runTest(MATHLIB_sin_testParams_t *pCurrPrm, size_t length, T *pSrc, T *pDst, uint32_t *pProfile)
{
   ...
   /* Cold run*/
   pDst[0] = MATHLIB_sin_scalar_ci<T>(pSrc[0]);
   pDst[0] = MATHLIB_sin_scalar_ci<T>(pSrc[0]);

   /* Warm run*/
   for (size_t ii = 0; ii < length; ii++) {
      MATHLIB_profile_start(MATHLIB_PROFILE_KERNEL_CN);
      pDst[ii] = MATHLIB_sin_scalar_ci<T>(pSrc[ii]);
      MATHLIB_profile_stop();
   }
...
int MATHLIB_sin_testBench(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   for (tpi = 0; tpi < (size_t) test_cases; tpi++) {
      ...
      if (currPrm.dType == MATHLIB_FLOAT32) {
         ...
         status = MATHLIB_sin_runTest(&currPrm, length, (double *) pSrc, (double *) pDst, &pProfile[0]);
      }
...
int coverage_test_main(uint32_t *pProfile)
{
   ...
   while (testNum <= 1007) {
      switch (testNum) {
      case 1000:
         status          = MATHLIB_sin(16, (float *) NULL, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1001:
         status          = MATHLIB_sin(16, pSrc_sp, (float *) NULL);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1002:
         status          = MATHLIB_sin(0, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1003:
         status          = MATHLIB_sin_sp(16, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1004:
         status          = MATHLIB_sin_dp(16, pSrc_dp, pDst_dp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1005:
         status          = MATHLIB_sin(0, pSrc_dp, pDst_dp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1006:
         pSrc_sp[0]      = 1.0f / 0.0f;
         pDst_sp[0]      = MATHLIB_sin_scalar_sp(pSrc_sp[0]);
         currentTestPass = (pDst_sp[0] == 0.0f);
         break;
      case 1007:
         pSrc_dp[0]      = 1.0f / 0.0f;
         pDst_dp[0]      = MATHLIB_sin_scalar_dp(pSrc_dp[0]);
...
int main(void)
{
   ...
   test_fail = MATHLIB_sin_testBench(&profile[0], 0);
