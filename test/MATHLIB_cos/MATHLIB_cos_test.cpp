...
int32_t MATHLIB_cos_runTest(MATHLIB_cos_testParams_t *pCurrPrm, size_t length, T *pSrc, T *pDst, uint32_t *pProfile)
{
   ...
   status_v = MATHLIB_compare_mem(pDst, (T *) pCurrPrm->staticOut, length, 10e-7, testMetric);
   ...
   /* Cold run */
   pDst[0] = MATHLIB_cos_scalar_ci<T>(pSrc[0]);
   pDst[0] = MATHLIB_cos_scalar_ci<T>(pSrc[0]);

   /* Warm run*/
   for (size_t ii = 0; ii < length; ii++) {
      MATHLIB_profile_start(MATHLIB_PROFILE_KERNEL_CN);
      pDst[ii] = MATHLIB_cos_scalar_ci<T>(pSrc[ii]);
      MATHLIB_profile_stop();
   }

   status_inline = MATHLIB_compare_mem(pDst, (T *) pCurrPrm->staticOut, length, 10e-7, testMetric);
...
int MATHLIB_cos_testBench(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   for (tpi = 0; tpi < (size_t) test_cases; tpi++) {
      ...
      if (currPrm.dType == MATHLIB_FLOAT32) {
         ...
         status = MATHLIB_cos_runTest(&currPrm, length, (double *) pSrc, (double *) pDst, &pProfile[0]);
      }
...
int coverage_test_main(uint32_t *pProfile)
{
   ...
   float  *pSrc_sp = (float *) MATHLIB_memalign(MATHLIB_L2DATA_ALIGNMENT, length * sizeof(float));
   float  *pDst_sp = (float *) MATHLIB_memalign(MATHLIB_L2DATA_ALIGNMENT, length * sizeof(float));
   double *pSrc_dp = (double *) MATHLIB_memalign(MATHLIB_L2DATA_ALIGNMENT, length * sizeof(double));
   double *pDst_dp = (double *) MATHLIB_memalign(MATHLIB_L2DATA_ALIGNMENT, length * sizeof(double));

   while (testNum <= 1007) {
      switch (testNum) {
      case 1000:
         status          = MATHLIB_cos(16, (float *) NULL, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1001:
         status          = MATHLIB_cos(16, pSrc_sp, (float *) NULL);
         currentTestPass = (status == MATHLIB_ERR_NULL_POINTER);
         break;
      case 1002:
         status          = MATHLIB_cos(0, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1003:
         status          = MATHLIB_cos_sp(16, pSrc_sp, pDst_sp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1004:
         status          = MATHLIB_cos_dp(16, pSrc_dp, pDst_dp);
         currentTestPass = (status == MATHLIB_SUCCESS);
         break;
      case 1005:
         status          = MATHLIB_cos(0, pSrc_dp, pDst_dp);
         currentTestPass = (status == MATHLIB_ERR_INVALID_DIMENSION);
         break;
      case 1006:
         pSrc_sp[0]      = 1.0f / 0.0f;
         pDst_sp[0]      = MATHLIB_cos_scalar_sp(pSrc_sp[0]);
         currentTestPass = (pDst_sp[0] == 1.0f);
         break;
      case 1007:
         pSrc_dp[0]      = 1.0f / 0.0f;
         pDst_dp[0]      = MATHLIB_cos_scalar_dp(pSrc_dp[0]);
         currentTestPass = (pDst_dp[0] == 1.0f);
         break;
      default:
         break;
      }
...
#endif
   MATHLIB_align_free(pSrc_sp);
   MATHLIB_align_free(pDst_sp);
   MATHLIB_align_free(pSrc_dp);
...
int main(void)
{
   ...
   test_fail = MATHLIB_cos_testBench(&profile[0], 0);
